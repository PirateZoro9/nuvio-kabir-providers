...
function normalizeMatchText(str) {
    return String(str || "")
        ...
        .replace(/[\u0300-\u036f]/g, '')
        .replace(/[^a-z0-9]+/g, ' ')
        .replace(/\b(tv|movie|anime|season|part|cour)\b/g, ' ')
        .replace(/\s+/g, ' ')
        .trim();
}
...
function seasonSearchQueries(title, season, seasonInfo) {
    ...
    if (seasonInfo && seasonInfo.name) queries.push(seasonInfo.name);
...
function scoreSearchItemTitle(itemTitle, queryTitle, season) {
    ...
    var score = 0;
    if (item === query) score += 100;
    if (item.indexOf(query) !== -1) score += 30;

    query.split(" ").forEach(function(token) {
        if (token && item.indexOf(token) !== -1) score += 8;
    });

    if (season > 1) {
        var seasonRegex = new RegExp("\\b(" + season + "|" + ordinalSeasonLabel(season) + ")\\b", "i");
        if (seasonRegex.test(itemTitle)) score += 45;
        if (!seasonRegex.test(itemTitle) && /season|part|cour/i.test(itemTitle)) score -= 30;
    }

    return score;
}

async function findBestAnimePaheSearchResult(title, season, seasonInfo) {
...
async function tmdbSeasonInfo(tmdbId, season) {
    if (!tmdbId || !season || Number(season) < 1) return null;
    try {
        var url = TMDB_BASE + "/tv/" + tmdbId + "/season/" + season
    ...
    try {
        ...
        if (loadData.tmdb_id && !loadData.session) {
            ...
            try {
                ...
                var seasonInfo = await tmdbSeasonInfo(loadData.tmdb_id, loadData.season);
                var searchResult = await findBestAnimePaheSearchResult(title, loadData.season || 1, seasonInfo);
                if (!searchResult) throw new Error("Not found on AnimePahe");
                loadData = Object.assign({}, originalLoadData, JSON.parse(searchResult.url));
                if (loadData && loadData.name == null) loadData.name = searchResult.title || title;
            } catch(e) {
        ...
        if (loadData.sessionDate && (loadData.sessionDate + 600 < now)) {
            ...
            var refreshSeasonInfo = loadData.tmdb_id ? await tmdbSeasonInfo(loadData.tmdb_id, loadData.season) : null;
            var searchRes = await findBestAnimePaheSearchResult(name, loadData.season || 1, refreshSeasonInfo);
            if (!searchRes) throw new Error("Session refresh failed");
...
async function fetchAllEpisodes(session, metaEpisodes) {
    ...
    try {
        ...
        episodes.sort(function(a, b) {
            ...
            if (a.dubStatus === b.dubStatus) return 0;
        ...
        if (mediaType === "movie") {
            ...
            targetEpisodes = loadResult.episodes.filter(function(ep) {
                return Number(ep.episode) === Number(episode) &&
                    (ep.season == null || Number(ep.season) === Number(season));
            });
