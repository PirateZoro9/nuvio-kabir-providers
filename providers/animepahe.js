 ...
 * Generated: 2026-04-17T07:52:34.065Z
 */
  ...
  return __async(this, null, function* () {
    try {
      ...
      if (loadData.tmdb_id && !loadData.session) {
        ...
        try {
          var searchResult = yield new Promise(function(resolve, reject) {
            search(title, function(r) {
              if (r.success && r.data.length > 0)
                resolve(r.data[0]);
              else
                reject(new Error("Not found on AnimePahe"));
            });
          });
      ...
      if (loadData.sessionDate && loadData.sessionDate + 600 < now) {
        var searchRes = yield new Promise(function(resolve, reject) {
          search(name, function(result2) {
            if (result2.success && result2.data.length > 0)
              resolve(result2.data[0]);
            else
